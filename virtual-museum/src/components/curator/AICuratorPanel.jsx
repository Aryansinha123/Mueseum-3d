...
export function AICuratorPanel({
  ...
  const executeAsk = async (qText) => {
    ...
    // Explicit debug logging as specified in audit requirements
  ...
  return (
    <div className={`flex flex-col h-full bg-slate-900/90 text-slate-100 p-4 space-y-4 rounded-xl border border-slate-800 ${className}`}>
        ...
        <div className="flex flex-wrap gap-1.5">
          {QUICK_QUESTIONS.map((qText) => (
            <button
              ...
              disabled={isLoading}
              className="px-2.5 py-1 rounded-lg text-[11px] bg-slate-950/70 hover:bg-amber-500/20 border border-slate-800 hover:border-amber-500/40 text-slate-300 hover:text-amber-200 font-medium transition-all disabled:opacity-50"
            >
              {qText}
            </button>
          ))}
      ...
      <div className="flex-1 overflow-y-auto space-y-3 min-h-[160px] max-h-[380px] pr-1">
        ...
        {isLoading && (
          <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-center gap-3 animate-pulse">
            ...
            <span className="text-xs font-bold text-amber-200">
              Curator is checking curated records & generating answer...
            </span>
          </div>
        )}
            ...
            <div className="flex items-center justify-between px-3 py-1.5 rounded-lg bg-slate-950/60 border border-slate-800 text-[11px]">
              <span className="text-slate-400 flex items-center gap-1 font-medium">
                ...
                <strong className={`font-mono ${response.refused ? "text-rose-400" : "text-emerald-400"}`}>
                  {typeof response.confidence === "number" ? response.confidence.toFixed(2) : response.confidence}
                </strong>
              </span>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {response.related_suggestions.map((rec) => (
                    <button
                      ...
                      className="p-2.5 rounded-xl bg-slate-950/60 hover:bg-slate-800 border border-slate-800 hover:border-amber-500/40 text-left transition-all flex items-center justify-between group"
                    >
      ...
      <form onSubmit={handleAsk} className="pt-2 border-t border-slate-800 flex items-center gap-2">
        <input
          ...
          className="flex-1 px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 focus:border-amber-500 text-xs text-slate-100 placeholder-slate-500 focus:outline-none transition-colors disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={!question.trim() || isLoading}
          aria-label="Send Question"
          className="px-3.5 py-2 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 disabled:opacity-40 text-slate-950 font-bold text-xs flex items-center gap-1.5 transition-all shadow-md shadow-amber-500/20 shrink-0"
        >
          <span>Ask</span>
